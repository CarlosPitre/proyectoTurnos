...
class TurnoControl{
  ...
  public function getReservaBySucursal(Request $request, Response $response)
  {
    ...
    $query = "SELECT t.*, c.nombres, c.apellidos, e.nombres as nombreEmpleado, e.apellidos as apellidoEmpleado FROM turno t INNER JOIN cliente c ON t.idCliente = c.id INNER JOIN empleado e ON t.idEmpleado = e.id  WHERE t.idServicio = '$idServicio' AND t.idSucursal = '$idSucursal' AND t.fechaReserva = '$fechaReserva'";
    $data = DB::select(DB::raw($query));
    $response->getBody()->write(json_encode($data));
    return $response;
  }
  ...
  public function getTurnoreserva(Request $request, Response $response){
    ...
    $query = "SELECT CONCAT( cliente.nombres,' ', cliente.apellidos ) AS cliente, turno.fechaReserva, turno.horaReserva, turno.horaFinalReserva,turno.id
            FROM turno
            INNER JOIN cliente ON cliente.id = turno.idCliente
            WHERE month(turno.fechaReserva) = '$mes' and year(turno.fechaReserva) = '$ano'
            AND turno.idSucursal = '$idSucursal' AND turno.estado <> 'INACTIVO'";
      $data = DB::select(DB::raw($query));
      $response->getBody()->write(json_encode($data));
      return $response;
  }
  ...
  public function getClienteByReserva(Request $request, Response $response)
  {
      ...
      $idSucursal = $request->getAttribute("idSucursal");
      $idCliente = $request->getAttribute("idCliente");
      $query = "SELECT t.id,t.fechaReserva, t.horaReserva,s.nombre as servicio, su.nombre as sucursal, CONCAT( e.nombres ,' ',e.apellidos) as empleado, t.serial 
        FROM turno t 
  ...
  public function deleteTurnos(Request $request, Response $response)      
  {
      $response = $response->withHeader('Content-type', 'application/json');
      $data = json_decode($request->getBody(),true);
      $serial = $data["serial"];
      $turnos = Turno::Select('*')->where("serial", "=", $serial)->get;
      for ($i=0; $i < count($turnos); $i++) { 
          $turnos[$i]->estado = "INACTIVO";
          $turnos[$i]->estadoTurno = "CANCELADO";
          $turnos[$i]->save();
      }
      /*$query = "UPDATE turno SET estado = 'INACTIVO', estadoturno = 'CANCELADO' WHERE serial = '$serial'";
      $data = DB::select(DB::raw($query));*/
      $respuesta = json_encode($turnos);
      $response = $response->withStatus(200);
      $response->getBody()->write($respuesta);
      return $response;
  }
